#!/bin/bash
# Prepares ptau, zkey and Solidity verifier for real ZK tests.
set -e
...
npx snarkjs zkey export solidityverifier "$BUILD/${CIRCUIT}_final.zkey" "$ROOT/src/HelloHashVerifier.sol"
sed -i.bak 's/pragma solidity [^;]*/pragma solidity ^0.8.20/' "$ROOT/src/HelloHashVerifier.sol"
sed -i.bak 's/contract Groth16Verifier/contract HelloHashVerifier/' "$ROOT/src/HelloHashVerifier.sol"
rm -f "$ROOT/src/HelloHashVerifier.sol.bak"
echo "ZK setup done."
